...
Ported from spx_ai_handover_v2/reference/test_temporal_harness.py, plus a
regression test for fail-closed handling of a delivery recorded at/before the
decision time for evidence that is not yet historically available.
"""
...
from spx_research.epistemics.harness import Harness, canonical
...
T = datetime(2019, 1, 2, 15, 0, tzinfo=UTC)
...
class HarnessReferenceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.menu = [
            MenuChoice("hold-own-position", "HOLD", ("rule", "rate"), "SPXW-PRIVATE-20190102")
        ]

    def delivery(self, atom_id, **kwargs):
        return replace(
            Delivery(self.ctx.run_id, self.ctx.branch_id, self.ctx.actor_id, atom_id, T), **kwargs
        )
    ...
    def proposal(self, compiled):
        ...
        return {
            ...
            "uncertainty_codes": ["UNKNOWN_FUTURE_POLICY_PATH"],
    ...
    def test_future_suffix_invariant(self):
        future = replace(
            ...
            atom_id="future",
            value="999",
            published_at=T + timedelta(days=2),
            available_at=T + timedelta(days=2),
            subject_at=T + timedelta(days=2),
        )
    ...
    def test_future_record_insertion_does_not_change_packet(self):
        ...
        self.assertEqual(
            self.compile().public, self.compile(dict(self.atoms, inserted=future)).public
        )

    def test_visible_change_positive_control(self):
        self.assertNotEqual(
            self.compile().public,
            self.compile(dict(self.atoms, rate=replace(self.rate, value="350"))).public,
        )

    def test_hidden_manifest_change_invariant(self):
        self.assertEqual(
            self.compile().public,
            self.compile(ctx=replace(self.ctx, private_manifest_id="different-future-v2")).public,
        )
    ...
    def test_known_future_schedule_permitted(self):
        a = replace(
            ...
            value="45",
            unit="minutes",
            subject_at=T + timedelta(minutes=45),
        )
        c = self.compile(
            dict(self.atoms, schedule=a), [*self.deliveries, self.delivery("schedule")]
        )
    ...
    def test_published_forecast_permitted(self):
        a = replace(
            self.rate,
            atom_id="forecast",
            kind="SOURCE_FORECAST",
            metric="expected_rate_bps",
            subject_at=T + timedelta(days=45),
        )
        c = self.compile(
            dict(self.atoms, forecast=a), [*self.deliveries, self.delivery("forecast")]
        )
    ...
    def test_delivery_recorded_for_not_yet_available_atom_rejected(self):
        ...
        self.assert_code(
            "DELIVERY_BEFORE_AVAILABILITY", lambda: self.compile(dict(self.atoms, fut=future), d)
        )
    ...
    def test_wrong_recipient_rejected(self):
        self.assert_code(
            "WRONG_RECIPIENT",
            lambda: self.compile(
                dict(self.atoms, rate=replace(self.rate, recipients=("other-worker",)))
            ),
        )
    ...
    def test_unverified_source_rejected(self):
        self.assert_code(
            "UNVERIFIED_SOURCE",
            lambda: self.compile(dict(self.atoms, rate=replace(self.rate, source_checked=False))),
        )
    ...
    def test_future_dependency_rejected(self):
        ...
        self.assert_code(
            "FUTURE_DEPENDENCY", lambda: self.compile(dict(self.atoms, rate=child, parent=parent))
        )
    ...
    def test_derived_number_recomputed(self):
        ...
        q = replace(
            self.rate,
            atom_id="change",
            metric="policy_delta_bps",
            value="25",
            kind="DERIVED",
            dependencies=("rate", "old"),
            transform="SUBTRACT",
        )
        c = self.compile(
            dict(self.atoms, old=p, change=q), [*self.deliveries, self.delivery("change")]
        )
    ...
    def test_nonfinite_value_rejected(self):
        self.assert_code(
            "NONFINITE_VALUE",
            lambda: self.compile(dict(self.atoms, rate=replace(self.rate, value="NaN"))),
        )
    ...
    def test_naive_time_rejected(self):
        self.assert_code(
            "NAIVE_TIME", lambda: self.compile(ctx=replace(self.ctx, as_of=T.replace(tzinfo=None)))
        )

    def test_accepted_witness_never_proves_model_ignorance(self):
        c = self.compile()
        w = self.h.validate(self.proposal(c), c)
    ...
    def test_unknown_evidence_token_rejected(self):
        c = self.compile()
        p = self.proposal(c)
        p["premise_tokens"].append("ev_invented")
    ...
    def test_valid_subjective_assessment_is_not_a_fact_write(self):
        ...
        before = copy.deepcopy(self.atoms)
        self.h.validate(p, c)
    ...
    def test_manager_has_same_prefix_boundary(self):
        ...
        second = self.compile(
            deliveries=ds, ctx=replace(ctx, private_manifest_id="another-future"), menu=menu
        )
    ...
    def test_role_inappropriate_menu_rejected(self):
        self.assert_code(
            "ACTION_WRONG_ROLE",
            lambda: self.compile(menu=[MenuChoice("allocate", "ALLOCATE", ("rule",))]),
        )
